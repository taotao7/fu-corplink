...
连接成功后 `http://localhost:8989` 即可作为 HTTP / SOCKS5 代理使用。
...
代理默认 listen 在 `0.0.0.0:8989`（HTTP / SOCKS5 混合），任何能连到这个端口的客户端都能用：
...
| `username` | (自动) | 登录用户名 |
| `socks_listen` | `0.0.0.0:8989` | 代理监听地址（HTTP / SOCKS5 混合） |
...
| `8989` | HTTP / SOCKS5 混合代理 | 仅当 VPN 已连接时 |
...
- 代理默认 `0.0.0.0:8989`，详见上面"代理暴露"段落
