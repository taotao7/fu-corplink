...
> ⚠️ **缓存在内存里，进程/容器重启即清空——重启后首次打开重资产页面会明显慢，这不是故障。** 冷加载时所有资产都要过隧道，而隧道吞吐受 WG-over-TCP + 频繁轮换限制（实测常只有 ~10-30KB/s），一个 1MB 的 SPA bundle 可能要 1-2 分钟才下完、页面才渲染。期间续传机制在后台持续累积进度（重试/刷新不会从零开始），耐心等待即可；缓存热了之后同页面所有加载都是毫秒级，且不再受隧道抖动影响。
...
> **改配置请走控制台或 `POST /api/config`，不要直接编辑 `config.json`。** 配置文件只在启动时读一次，运行时权威是进程内存；`route_mode`、`force_protocol` 等是连接时读取的，而断开/重连/上报都会触发一次内存→磁盘的 `Save()`。进程运行期间直接改文件，会被内存里的旧值覆盖回去（下次 `Save()` 就盖没了），表现为改了不生效、过一会儿又变回去。正确做法：用控制台"设置"或 `POST /api/config` 改（同时更新内存和磁盘），改完断开重连让需要重建隧道的项（如 `route_mode`）生效。直接编辑文件仅在**停机状态**下可靠。
...
- 飞连上游接口可能变化，长期兼容性不保证。
- 资产缓存是纯内存的：**重启进程/容器后，首次访问重资产内网页面（大 JS bundle）需 1-2 分钟冷加载**，之后毫秒级（见[隧道自愈机制](#隧道自愈机制)第 5 条）。
